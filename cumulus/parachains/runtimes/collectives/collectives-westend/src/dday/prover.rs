...
impl VerifyProof for AssetHubAccountProver {
	...
	fn query_voting_power_for(
		...
		Some(VotingPower { account_power: account_balance.data.total(), total: total_issuance })
	}
}
				...
				.match_next_inst(|inst| match inst {
					Transact { call, .. } => {
						...
						match call.ensure_decoded().map_err(|_| ProcessMessageError::BadFormat)? {
							super::RuntimeCall::ProofsStorage(..) => Ok(()),
							_ => Err(ProcessMessageError::BadFormat),
						}
					},
					_ => Err(ProcessMessageError::BadFormat),
				})?
				.skip_inst_while(|inst| matches!(inst, ExpectTransactStatus(..)))?
...
pub mod tests {
	...
	/// 	key2: 0x26aa394eea5630e07c48ae0c9558cef7b99d880ec681799c0cf30e8886371da9942ba479328ac3d026b2c4cced7e2508f0709f5078496e42469e70752483d4f820702bb01e335d9e03bad0b54f729251
	...
	/// }
	pub fn sample_proof(
	) -> (parachains_common::Header, RawStorageProof, &'static str, Vec<u8>, Vec<u8>) {
		...
		(header, proof, ss58_account, account_balance_key, total_issuance_key)
	}
	...
	fn asset_hub_account_prover_works() {
		...
		sp_io::TestExternalities::new(Default::default()).execute_with(|| {
			// prepare proof
			let (asset_hub_header, proof, ss58_account, account_balance_key, total_issuance_key) =
				sample_proof();
